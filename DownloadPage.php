...
if (empty("$DownloadLink")) exit;
else if (!file_exists($DownloadTurePath)) exit;
else {
...
<div class="mdui-container" style="max-width: 400px; ">
	...
	<div class="mdui-card">
		...
		<div class="mdui-card-primary">
			...
			<div class="mdui-card-primary-subtitle"><?php echo date("Y-m-d H:i:s", (filemtime("$DownloadTurePath")));
													echo '&nbsp;&nbsp;&nbsp;';
													echo SizeUnit(filesize($DownloadTurePath)); ?></div>
		</div>

		<div class="mdui-card-content"><?php if (empty($Introduction)) echo '不解释，你应该懂的，2333333';
										else echo $Introduction; ?><br><?php if (!empty($IntroductionO)) echo $IntroductionO; ?></div>
...
<script>
	...
	function DownloadNow() {
		window.location.href = "<?php if ($Rewrite == '1') echo 'http://' . str_replace("//", "/", dirname($_SERVER['SERVER_NAME'] . ':' . $_SERVER["SERVER_PORT"] . $_SERVER['PHP_SELF']) . '/' . $DownloadLink);
								else echo '?Download=' . $DownloadLink ?>";
	}
</script>